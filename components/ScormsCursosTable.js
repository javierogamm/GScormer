...
const getScormReferenceLabelsFromContenido = (contenido) => {
  return extractScormReferencesFromContenido(contenido)
    .map((reference) => {
...
const resolveScormReferenceDiff = (previousLabels = [], nextLabels = []) => {
  const previousMap = new Map();
  const nextMap = new Map();

  previousLabels.forEach((label) => {
    const normalized = normalizeScormReferenceLabel(label);
    if (normalized && !previousMap.has(normalized)) {
      previousMap.set(normalized, String(label || '').trim());
    }
  });
  ...
  nextLabels.forEach((label) => {
    ...
    if (normalized && !nextMap.has(normalized)) {
      nextMap.set(normalized, String(label || '').trim());
    }
  });

  return {
    added: Array.from(nextMap.entries()).filter(([key]) => !previousMap.has(key)).map(([, label]) => label),
...
export default function ScormsCursosTable({ userSession }) {
  ...
  const canAccessScormChangeAlerts = userSession?.admin === true;
  ...
  const fetchData = useCallback(async () => {
    ...
    if (cursosResponse.error) {
      ...
      setRows(loadedRows);
      setStatusMessage(`Cursos cargados: ${loadedRows.length}`);
    }
    ...
    setLoading(false);
  }, [canAccessScormChangeAlerts, userSession?.id, userSession?.name]);
  ...
  useEffect(() => {
    if (!canAccessScormChangeAlerts && cursosSubView === 'alertas_scorms') {
      setCursosSubView('general');
    }
  }, [canAccessScormChangeAlerts, cursosSubView]);
  ...
  const saveDetailModal = async () => {
    ...
    const scormUpdateResponse = await recordScormCourseUpdate({
    ...
    if (scormUpdateResponse.error) {
      setDetailSaving(false);
      setError(`El curso se actualizó, pero no se pudo registrar la alerta de cambios de SCORMs: ${scormUpdateResponse.error.message}`);
      return;
    }
  ...
  const submitCreateCurso = async () => {
    ...
    const scormUpdateResponse = await recordScormCourseUpdate({
      courseRow: response.data,
      added: getScormReferenceLabelsFromIds(selectedScormIds, masterRows),
      removed: [],
    });

    if (scormUpdateResponse.error) {
      setCreateSubmitting(false);
      setError(`El curso se creó, pero no se pudo registrar la alerta de SCORMs añadidos: ${scormUpdateResponse.error.message}`);
      return;
    }
  ...
  const applyLookupFilterValues = (columnKey, selectedValues) => {
    setFilters((previous) => {
  ...
  const applyLookupFilter = (columnKey) => {
    applyLookupFilterValues(columnKey, filterDraftSelections[columnKey] || []);
  };

  const applyMatchingLookupFilter = (columnKey) => {
    const normalizedSearch = normalizeText(filterLookupSearchInputs[columnKey]);
    const matchingOptions = (selectorFilterOptions[columnKey] || []).filter((option) => {
      if (!normalizedSearch) {
        return true;
      }
    ...
    applyLookupFilterValues(columnKey, selectedValues);
  };
  ...
  return (
    <section className="card card-wide">
      <div className="card-header">
        <div>
          <h2>CURSOS · {cursosSubViewTitle}</h2>
                  ...
                  {courseScormChangeAlerts.map((alert) => {
                    return (
                      <tr key={`scorm-change-${alert.id}`} onDoubleClick={() => setScormChangeDetails(alert)}>
                        ...
                        <td>{String(alert.row.curso_instructor || '-')}</td>
      ...
      {scormChangeDetails ? (
        <div className="modal-overlay" role="presentation">
          <section className="modal-content modal-content-narrow" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="scorm-change-detail-grid">
              <section className="scorm-change-panel added">
                <h4>SCORMs añadidos ({scormChangeDetails.added.length})</h4>
                {scormChangeDetails.added.length === 0 ? (
                  <p className="status">Sin SCORMs añadidos.</p>
                ) : (
                  <ul className="scorm-change-list">
                    {scormChangeDetails.added.map((reference) => (
                      <li key={`added-${reference.key}`}>
                        <strong>{reference.label}</strong>
                        {reference.name ? <span>{reference.name}</span> : null}
                      </li>
                    ))}
                  </ul>
                )}
              ...
              <section className="scorm-change-panel removed">
                <h4>SCORMs eliminados ({scormChangeDetails.removed.length})</h4>
                {scormChangeDetails.removed.length === 0 ? (
                  <p className="status">Sin SCORMs eliminados.</p>
                ) : (
                  <ul className="scorm-change-list">
                    {scormChangeDetails.removed.map((reference) => (
                      <li key={`removed-${reference.key}`}>
                        <strong>{reference.label}</strong>
                        {reference.name ? <span>{reference.name}</span> : null}
                      </li>
                    ))}
